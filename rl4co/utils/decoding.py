...
def random_policy(td):
    ...
    td.set("action", action)  # type: ignore
    return td


def rollout(env, td, policy, max_steps: int | float = None):  # type: ignore
    ...
    max_steps = float("inf") if max_steps is None else max_steps  # type: ignore
    actions = []
    steps = 0
...
def process_logits(
    logits: torch.Tensor,
    mask: torch.Tensor | None = None,  # type: ignore
    ...
    if mask_logits:
        ...
        logits[~mask] = float("-inf")  # type: ignore
...
class DecodingStrategy(metaclass=abc.ABCMeta):
        ...
        if self.num_starts >= 1:
            if self.multistart:
                ...
                if self.store_all_logp:
                    logprobs = torch.zeros_like(td["action_mask"])  # type: ignore  # [B, N]
                else:
                    logprobs = torch.zeros_like(action, device=td.device)  # type: ignore  # [B]
    ...
    ) -> tuple[torch.Tensor, torch.Tensor, TensorDict, RL4COEnvBase]:  # type: ignore
        assert len(self.logprobs) > 0, (
            "No logprobs were collected because all environments were done. Check your initial state"
        )
        logprobs = torch.stack(self.logprobs, 1)  # type: ignore
        actions = torch.stack(self.actions, 1)  # type: ignore
        if self.num_starts > 0 and self.select_best:  # type: ignore
            logprobs, actions, td, env = self._select_best(logprobs, actions, td, env)  # type: ignore
        return logprobs, actions, td, env  # type: ignore
    ...
    def step(
        ...
        **kwargs,
        ...
        if self.improvement_method_mode:
            return logprobs, selected_action  # type: ignore
        ...
        td.set("action", selected_action)  # type: ignore
    ...
    def _select_best(self, logprobs, actions, td: TensorDict, env: RL4COEnvBase):  # type: ignore
        rewards = env.get_reward(td, actions)  # type: ignore
        _, max_idxs = unbatchify(rewards, self.num_starts).max(dim=-1)  # type: ignore

        actions = unbatchify_and_gather(actions, max_idxs, self.num_starts)  # type: ignore
        logprobs = unbatchify_and_gather(logprobs, max_idxs, self.num_starts)  # type: ignore
        td = unbatchify_and_gather(td, max_idxs, self.num_starts)  # type: ignore
...
class BeamSearch(DecodingStrategy):
    ...
    def _step(
        ...
        assert not (~mask).gather(1, selected.unsqueeze(-1)).data.any(), (  # type: ignore
            "infeasible action selected"
        )
    ...
    def pre_decoder_hook(self, td: TensorDict, env: RL4COEnvBase, **kwargs):
        ...
        assert self.beam_width is not None and self.beam_width > 1, "beam width must be larger than 1"
        ...
        logprobs = torch.zeros_like(td["action_mask"], device=td.device)  # type: ignore
    ...
    def post_decoder_hook(self, td, env):  # type: ignore
        # [BS*BW, seq_len]
        aligned_sequences, aligned_logprobs = self._backtrack()

        if self.select_best:
            return self._select_best_beam(aligned_logprobs, aligned_sequences, td, env)  # type: ignore
        else:
            return aligned_logprobs, aligned_sequences, td, env  # type: ignore

    def _backtrack(self) -> tuple[torch.Tensor, torch.Tensor]:  # type: ignore
        # [BS*BW, seq_len]
        actions = torch.stack(self.actions, 1)  # type: ignore
        # [BS*BW, seq_len]
        logprobs = torch.stack(self.logprobs, 1)  # type: ignore
        ...
        batch_size = aug_batch_size // int(self.beam_width)  # type: ignore
        batch_beam_sequence = torch.arange(0, batch_size).repeat(int(self.beam_width)).to(actions.device)  # type: ignore
        ...
        actions = torch.stack(list(reversed(reversed_aligned_sequences)), dim=1)  # type: ignore
        logprobs = torch.stack(list(reversed(reversed_aligned_logprobs)), dim=1)  # type: ignore

        return actions, logprobs  # type: ignore

    def _select_best_beam(self, logprobs, actions, td: TensorDict, env: RL4COEnvBase):  # type: ignore
        aug_batch_size = logprobs.size(0)  # num nodes
        batch_size = aug_batch_size // self.beam_width  # type: ignore
        rewards = env.get_reward(td, actions)  # type: ignore
        _, idx = torch.cat(rewards.unsqueeze(1).split(batch_size), 1).max(1)  # type: ignore
        flat_idx = torch.arange(batch_size, device=rewards.device) + idx * batch_size  # type: ignore
        return logprobs[flat_idx], actions[flat_idx], td[flat_idx], env  # type: ignore

    def _make_beam_step(self, logprobs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:  # type: ignore
        assert self.beam_width is not None, "beam_width must be set before _make_beam_step"  # type: ignore
        aug_batch_size, num_nodes = logprobs.shape  # num nodes  # type: ignore
        batch_size = aug_batch_size // int(self.beam_width)  # type: ignore
        batch_beam_sequence = (
            torch.arange(0, batch_size).repeat(int(self.beam_width)).to(logprobs.device)  # type: ignore
        )

        # [BS*BW, num_nodes] + [BS*BW, 1] -> [BS*BW, num_nodes]
        parent_beam_logprobs = (
            self.parent_beam_logprobs
            if self.parent_beam_logprobs is not None
            else torch.zeros_like(logprobs[:, :1])
        )
        log_beam_prob = logprobs + parent_beam_logprobs  # type: ignore

        # [BS, num_nodes * BW]
        log_beam_prob_hstacked = torch.cat(log_beam_prob.split(batch_size), dim=1)
        # [BS, BW]
        topk_logprobs, topk_ind = torch.topk(log_beam_prob_hstacked, int(self.beam_width), dim=1)  # type: ignore
